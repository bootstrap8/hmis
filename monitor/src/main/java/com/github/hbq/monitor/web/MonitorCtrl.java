...
public class MonitorCtrl {
  ...
  @ApiOperation("查询所有指标通知规则")
  @Version("v1.0")
  @RequestMapping(path = "/queryAllQuotaWarnRules/{v}", method = RequestMethod.POST)
  @ResponseBody
  public Result<?> queryAllQuotaWarnRules(
      @RequestHeader(name = "userInfo", required = false) String userInfo,
      @ApiParam(required = true, defaultValue = "v1.0") @PathVariable String v,
      @RequestBody Map map) {
    log.info("查询所有指标通知规则: {}", map);
    try {
      map.put("enable", -1);
      return Result.suc(monitorService.queryQuotaWarnRules(map));
    } catch (Exception e) {
      log.error("查询所有指标通知规则异常", e);
      return (e instanceof RuntimeException) ?
          Result.fail(e.getMessage()) :
          Result.fail("查询所有指标通知规则异常");
    }
  }
  ...
  public Result<?> queryQuotaInfos(
    ...
    try {
      Page page = PageHelper.startPage(pageNum, pageSize);
      List<Map> qis = agentService.queryQuotaInfos(map, pageNum, pageSize);
      List<QuotaWarnRuleInfo> ruleInfos = monitorService.queryQuotaWarnRules(ImmutableMap.of("enable", -1));
      Map<String, QuotaWarnRuleInfo> rm = ruleInfos.stream().collect(Collectors.toMap(rule -> rule.getKey(), rule -> rule, (r1, r2) -> r2));
      for (Map qi : qis) {
        String key = String.join(",", MapUtils.getString(qi, "appName"), MapUtils.getString(qi, "quotaName"));
        qi.put("hasThreshold", rm.containsKey(key) ? "是" : "否");
      }
      PageInfo<Map> pageInfo = new PageInfo<>(qis);
      pageInfo.setTotal(page.getTotal());
      return Result.suc(pageInfo);
    } catch (Exception e) {
